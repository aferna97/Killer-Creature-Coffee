if (instance_exists(obj_cauldron_top)){
	
	//debug
	draw_set_halign(fa_center)

	/*draw_text(50, 50, "bean: " + string(obj_cauldron_top.brew_bean_1))
	draw_text(50, 70, "bean2: " + string(obj_cauldron_top.brew_bean_2))
	draw_text(50, 90, "bean3: " + string(obj_cauldron_top.brew_bean_3))
	draw_text(50, 110, "dry1: " + string(obj_cauldron_top.brew_dry_1))
	draw_text(50, 130, "dry2: " + string(obj_cauldron_top.brew_dry_2))
	draw_text(50, 150, "dry3: " + string(obj_cauldron_top.brew_dry_3))
	draw_text(50, 170, "dry4: " + string(obj_cauldron_top.brew_dry_4))
	draw_text(50, 190, "dry5: " + string(obj_cauldron_top.brew_dry_5))
	draw_text(50, 210, "dry6: " + string(obj_cauldron_top.brew_dry_6))
	draw_text(50, 230, "dry7: " + string(obj_cauldron_top.brew_dry_7))
	draw_text(50, 250, "dry8: " + string(obj_cauldron_top.brew_dry_8))
*/
}
var count = maxTime - seconds
if((count <= 59) && (count >= 51))
{	
	draw_set_font(Fnt_dialogue_cat)
	draw_text_transformed(650,50,"Hint: click the filter first to display over cup, \nthen pour brewed coffee over into cup to filter",2,2,0)
	//draw_sprite(spr_cat_small,1,310,60)
}